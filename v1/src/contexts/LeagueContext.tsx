...
export function LeagueProvider({children}: LeagueProviderProps) {
    ...
    const createLeague = async (name: string, description?: string): Promise<League> => {
        ...
        // Automatically fetch odds for the new league
        try {
            console.log('Triggering odds sync for new league...');
            ...
            if (response.ok) {
                ...
                console.log('Odds sync completed:', result);
            } else {
                console.warn('Odds sync failed, but league created successfully');
            }
        } catch (error) {
            console.warn('Failed to sync odds automatically:', error);
            // Don't throw here - league creation was successful
        }
